...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Сколько пар кадров выбросить после применения параметров (стабилизация драйвера).",
    )
    ...
    def discard_frame_pair(n: int) -> None:
    ...
    if saved_cam:
        ...
        # Важно: не читать exposure/gain из cap.get() на старте — в авто-режиме это случайные числа,
        # из-за них яркость будет «плавать» при каждом открытии. Берём те же дефолты, что в capture.
        exp = float(np.clip(CAM_DEFAULT_EXPOSURE, EXPOSURE_MIN, EXPOSURE_MAX))
        gain = int(np.clip(CAM_DEFAULT_GAIN, GAIN_MIN, GAIN_MAX))
        bri = int(np.clip(CAM_DEFAULT_BRIGHTNESS, 0, PROP_RANGE))
        con = int(np.clip(CAM_DEFAULT_CONTRAST, 0, PROP_RANGE))
        gam = int(np.clip(CAM_DEFAULT_GAMMA, 0, PROP_RANGE))
        print(
            f"Файл {CAMERA_PARAMS_FILE} не найден или пуст — старт с дефолтами "
            f"(как в capture): exp={exp} gain={gain} bri={bri} con={con} gam={gam}"
        )
    ...
    discard_frame_pair(args.warmup_frames)
    apply_camera_params(exp, gain, bri, con, gam)
